package com.jdh.urlsaver.oauth.handler;
...
import static com.jdh.urlsaver.oauth.repository.OAuth2AuthorizationRequestBasedOnCookieRepository.*;
...
public class OAuth2AuthenticationSuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
    ...
    protected String determineTargetUrl(HttpServletRequest request, HttpServletResponse response,
        ...
        // DB 저장
        Token token = userRefreshTokenRepository.findByUserId(userInfo.getId());
        if (token != null) {
            token.setRefreshToken(refreshToken.getToken());
        } else {
            token = new Token(userInfo.getId(), refreshToken.getToken());
